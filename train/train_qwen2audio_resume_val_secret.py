...
class SFT_dataset(Dataset):
    def __init__(self,args,processor):
        ...
        #choose data source
        sources=list(self.data_dict.keys())#[1:] if len(list(self.data_dict.keys()))>1 else list(self.data_dict.keys())
        #data source
...
class SFTMetric:
    ...
    def get_metric(self, reset=True):
        ...
        rank=int(os.getenv('RANK',-1))
        if rank==0 or rank==-1:
            if isinstance(loss,torch.Tensor) and torch.isnan(loss) or math.isnan(loss) or np.isnan(loss):
                print(f'acc:{self.right.item()}/{self.total.item()}')
                print(f'loss:{self.total_loss.item()}/{self.world_size * self.n_step}')
...
def train(args):
    ...
    # Train AudioEncoder, Projector
    # print(type(config.audio_config.encoder_layers))
    # print(config.audio_config.encoder_layers)
    # if args.freeze:
    ...
    # #Train AudioEncoder first 2 layers + Projector
    # if args.freeze:
    #     train_front_layers=list(range(config.audio_config.encoder_layers))[:2]
    #     train_back_layers=list(range(config.audio_config.encoder_layers))[-2:]
    #     train_layers=train_front_layers+train_back_layers
    #     for name,params in model.named_parameters():
    #         params.requires_grad=False
    #         if 'multi_modal_projector' in name:
    #             params.requires_grad =True
    #         for layer_id in train_layers:
    #             # if f'audio_tower.layers.{layer_id}' in name:
    #             #     params.requires_grad=True
    #             if f'audio_tower.layers.{layer_id}.self_attn' in name:
    #                 params.requires_grad=True


    #Train AudioEncoder all encoder + Projector
    if args.freeze:
        # train_front_layers=list(range(config.audio_config.encoder_layers))[:2]
        # train_back_layers=list(range(config.audio_config.encoder_layers))[-2:]
        # train_layers=train_front_layers+train_back_layers
    ...
    #Train AudioEncoder last 2 layers Attn + Projector
    # print(type(config.audio_config.encoder_layers))
    # print(config.audio_config.encoder_layers)

    # if args.freeze:
    #     train_layers=list(range(config.audio_config.encoder_layers))[-2:]
    #     for name,params in model.named_parameters():
    #         params.requires_grad=False
    #         if 'multi_modal_projector' in name:
    #             params.requires_grad =True
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for batch_cnt, batch in train_dataloader_iterator:
            ...
            #test:print grad
            # batch_cnt_list=[1,2,3]
            # if batch_cnt in batch_cnt_list and epoch == 0:
            #     accelerator.print(f'batch_cnt={batch_cnt}')
            #     for name,param in model.named_parameters():
            #         if param.grad is not None:
            #             accelerator.print(f'Start steps: name:{name},grad:{param.grad}')
            ...
            if accelerator.is_main_process:
                ...
                #test:print grad
                # if isinstance(train_loss,torch.Tensor) and torch.isnan(train_loss) or math.isnan(train_loss) or np.isnan(train_loss):
                #     accelerator.print(type(train_loss))
                #     for name,param in model.named_parameters():
                #         if param.grad is not None:
                #             accelerator.print(f'Nan steps: name:{name},grad:{param.grad}')
