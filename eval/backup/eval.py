...
def eval(args):
    ...
    for audio_file in tqdm(audio_files):    
        ...
        #                     sr=processor.feature_extractor.sampling_rate)[0])
                        # audio_data, sample_rate = librosa.load("audio_url", sr=None)
        #name age sex emotion asr
        result=[audio_file]
        for text in texts:
            inputs = processor(text=text, audios=audios,sampling_rate=sr, return_tensors="pt", padding=True)
